module Usage
  class Parsed
    attr_reader(*%i[clauses command_keys external negated occurrence_counts sources values])

    def initialize(command_keys:, values:, occurrences:, negated:, sources:, clauses:, external: [])
      @clauses = clauses
    ...
    def boolean(key)
      ...
      when :env then Parser.env_truth?(values.fetch(key).last)
  ...
  class Parser
    ...
    def initialize(root, metadata, args, env: ENV)
      @ancestors = []
      @argument_filled = false
      @argument_position = @argument_taken = 0
      @argv = args.dup
      @bound = {}
      @bundle = ""
      @clause_values = {}
    ...
    def parse
      step while @position < @argv.length || !@bundle.empty?
      finish_clause
      if command.arg_required_else_help && @command_starts[command.key] == @argv.length
    ...
    def step
      ...
      if command.subcommand_precedence_over_argument && !@flags_stopped &&
          (subcommand = find_named(command, @argv[@position]))
        conflict!(@argv[@position]) if command.arguments_conflict_with_subcommands && @command_argument_found
        @position += 1
        descend(subcommand)
        return
      end

      if @collecting && @position < @argv.length
        token = @argv[@position]
      ...
      return if @position >= @argv.length

      token = @argv[@position]
      @position += 1
      if !@separator_seen && command.clause && token == command.clause.separator
        finish_clause
        @argument_position = @argument_taken = 0
        @argument_filled = false
        @collecting = nil
      ...
      if token == "--"
        ...
        @flags_stopped = @separator_seen = true
        required = current_arguments.each_index.find do |index|
          index >= @argument_position && current_arguments[index].double_dash == :required
        end
        @argument_position, @argument_taken = required, 0 if required
        return
      end

      if @argument_taken.positive? && (arg = next_argument) && present?(arg.value_terminator) && token == arg.value_terminator
        advance_argument
        return
      end
      ...
      if negative_number?(token) && !numeric_short
        ...
        return word(token) if command.external_subcommand && !@argument_filled
      end

      if flag_like?(token)
        return long_flag(token) if token.start_with?("--")
        unless bundle_known?(token)
          raise Error, "unknown flag: #{token}" if command.unknown_flags == :error
          return word(token)
        end
    ...
    def long_flag(token)
      ...
      raise Error, "version requested" if name == "version" && command.version && !command.disable_version_flag
      raise Error, "help requested" if name == "help" && !command.disable_help_flag
      raise Error, "unknown flag: #{token}" if command.unknown_flags == :error
      word(token)
    end
    ...
    def action!(flag)
      case flag.action
      when :help, :help_short, :help_long, :help_all then raise Error, "help requested"
      when :version then raise Error, "version requested"
      else bind_flag(flag, nil, false)
      end
    end

    def word(token)
      unless @argument_filled || @flags_stopped
        if (subcommand = find_named(command, token))
          conflict!(token) if command.arguments_conflict_with_subcommands && @command_argument_found
          return descend(subcommand)
        end
        if token == "help" && !command.disable_help_subcommand && !command.subcommands.empty?
          help_command = command
          while @position < @argv.length && (subcommand = find_named(help_command, @argv[@position]))
            help_command = subcommand
            @position += 1
          end
          raise Error, "help requested"
        end
        default = command.default_subcommand
        accepts_negative = default && negative_number?(token) && default.arguments.first&.allow_negative_numbers
        if default && !@default_taken && (!flag_like?(token) || accepts_negative) && token != "--"
          @default_taken = true
          descend(default)
          @position -= 1
          @command_starts[default.key] = @position
          return
        end
        return bind_sigil(token) if match_sigil(token)
        if command.external_subcommand && (!flag_like?(token) || negative_number?(token)) && !%w[-- -].include?(token)
          @external = [token, *@argv[@position..]]
          @position = @argv.length
          return
        end
      end

      return bind_sigil(token) if @argument_filled && match_sigil(token)

      skip_sigil_arguments
      reserve_for_required
      arg = next_argument
      raise Error, "unexpected argument: #{token}" unless arg
      raise Error, "argument requires a -- separator: #{arg.name}" if arg.double_dash == :required && !@separator_seen

      @argument_filled = @command_argument_found = true
      trailing = @separator_seen || arg.double_dash == :automatic
      delimit = !(@dont_delimit_trailing_values && trailing)
      @flags_stopped = true if arg.double_dash == :automatic
      if arg.variadic
        @argument_taken += values_in(token, delimit ? arg.delimiter : nil)
        advance_argument if arg.var_max.to_i.positive? && @argument_taken >= arg.var_max
      else
        advance_argument
      end
    ...
    def bind_flag(flag, value, has_value, negated = false)
      @seen += 1
      bound = (@bound[flag.key] ||= Bound.new)
      bound.occurrences += 1
      bound.negated = negated
      bound.at = @seen
      bound.values.concat(split_value(value, flag.delimiter)) if has_value
      @command_argument_found = true
    end

    def bind_argument(arg, value, delimit = true)
      @seen += 1
      target = command.clause ? @clause_values : @bound
      bound = (target[arg.key] ||= Bound.new)
      bound.occurrences += 1
      bound.at = @seen
      bound.values.concat(split_value(value, delimit ? arg.delimiter : nil))
    end
    ...
    def descend(command)
      ...
      @ancestors << self.command
      @command = command
      @command_path << command
      @command_starts[command.key] = @position
      @dont_delimit_trailing_values ||= command.dont_delimit_trailing_values
      @argument_position = @argument_taken = 0
      @argument_filled = @command_argument_found = false
    end

    def finish_clause
      return unless command.clause

      @clauses[command.key] << @clause_values
      @clause_values = {}
    end
    ...
    def resolve
      ...
      @command_path.each_with_index do |command, index|
        check_requirements = index == @command_path.length - 1 || !command.subcommand_negates_requirements
      ...
      Parsed.new(command_keys: @command_path.map(&:key), values: values,
    ...
    def apply_overrides
      lost = {}
      @bound.each do |key, bound|
        @metadata[key]&.fetch(:overrides, [])&.each do |other|
          next unless @bound[other]
          if @bound[other].at < bound.at then lost[other] = true
          elsif bound.at < @bound[other].at then lost[key] = true
          end
        end
      end
    ...
    def relationship_values(meta, item)
      ...
      when :env then [self.class.env_truth?(item[:values].last).to_s]
    ...
    def resolved_clauses(final)
      @command_path.each_with_object({}) do |command, clauses|
        next unless command.clause

        scope = command.clause.arguments.map(&:key)
        clauses[command.key] = @clauses.fetch(command.key, []).map do |instance|
    ...
    def current_arguments = command.clause ? command.clause.arguments : command.arguments
    def next_argument = current_arguments[@argument_position..]&.find { !present?(_1.sigil) }
    ...
    def skip_sigil_arguments
      @argument_position += 1 while @argument_position < current_arguments.length && present?(current_arguments[@argument_position].sigil)
    end

    def reserve_for_required
      return unless command.allow_missing_positional && @argument_taken.zero?
      while (arg = current_arguments[@argument_position]) && !arg.required
        required = current_arguments[(@argument_position + 1)..].count { _1.required && !present?(_1.sigil) }
        return if required.zero?
        remaining = 1 + @argv[@position..].count { (@flags_stopped || !flag_like?(_1)) && !match_sigil(_1) }
    ...
    def match_sigil(token)
      ...
      args = current_arguments + @ancestors.reverse.flat_map(&:arguments)
      arg = args.select { present?(_1.sigil) && token.start_with?(_1.sigil) }.max_by { _1.sigil.length }
      arg && [arg, arg.sigil]
    end

    def flags_in_scope
      command.flags + @ancestors.reverse.flat_map { _1.flags.select(&:global) }
    end
    ...
    def find_short(letter)
      flags_in_scope.find { _1.shorts.include?(letter) } ||
        (HELP_SHORT if letter == "h" && !command.disable_help_flag) ||
        (VERSION_SHORT if letter == "V" && command.version && !command.disable_version_flag)
    end

    def find_named(command, name)
      command.subcommands.find { _1.name == name } || command.subcommands.find { _1.aliases.include?(name) }
    end
